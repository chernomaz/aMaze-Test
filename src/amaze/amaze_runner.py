...
def main():
    ...
    policy_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
        os.path.dirname(__file__), "..", "..", "examples", "policies", "policy.json"
    )
    ...
    # Merge graph failures and policy violations into assertion_failures so they appear in the report
    extra_failures = graph_failures + ([str(policy_violation)] if policy_violation else [])
    if script_error:
        extra_failures.append(f"Script error: {script_error}")
    existing = set(runtime.assertion_failures)
    runtime.assertion_failures = runtime.assertion_failures + [f for f in extra_failures if f not in existing]
    expected_pass_env = os.environ.get("EXPECTED_PASS", "")
    expected_pass = bool(int(expected_pass_env)) if expected_pass_env in ("0", "1") else None
    runtime.write(expected_pass=expected_pass)
