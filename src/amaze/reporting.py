...
def build_report_model(audit: dict) -> dict:
    ...
    expected_pass = audit.get("expected_pass", None)
...
def _render_html_legacy(report: dict) -> str:
    ...
    # When the test failed, replace final_answer with violation details
    failure_details_parts = []
    for row in report["violations"]:
        d = row["details"]
        if isinstance(d, dict):
            d = d.get("message") or d.get("details") or str(d)
        if d:
            failure_details_parts.append(str(d))
    final_answer_label = "Final Answer"
    final_answer_text = s["final_answer"]
    if not passed and failure_details_parts:
        final_answer_label = "Failure Details"
    ...
    <div class="grid-summary">
      ...
      <div class="card">
        ...
        <div class="kvs">
          ...
          {('<div class="k">Expected</div><div><span class="badge ' + ('ok-badge">pass' if s['expected_pass'] else 'fail">fail') + '</span></div>') if s['expected_pass'] is not None else ''}
        ...
        <div class="prompt-label">{final_answer_label}</div>
        <div class="codeblock"><pre>{_esc(final_answer_text) or '<em style="color:var(--muted)">—</em>'}</pre></div>
      </div>
